...
const webIO = new WebSocket("ws://192.168.1.129:52125");
...
const makeOptions = () => {
    ...
    let hourDefaultOption = '<option value="default" class="default" selected>시(hour)</option>';
    let minuteDefaultOption = '<option value="default" class="default" selected>분(minute)</option>';
    ...
    hourSelect.forEach((element) => {
        ...
        element.addEventListener("focusout", (e) => {
            ...
            for(let i = 0 ; i < 60 ; i++){
                let flag = false;
                if(appointedTime[hourVal].includes(String(i))){// || i < minute){
                    flag = true;
                }
...
const verifyDiagPos = () => {
    diagPosImg.forEach((e) => {
        e.src = (diagPos) ? "./image.resize/diagPos.reduct.png" : "./image.resize/diagImpos.reduct.png";
    });
};
...
applyBtns.forEach((element) => {
    element.addEventListener("click", async (e) => {
        ...
        let data = {
            ...
            "time":[applyForm.getElementsByClassName('hour')[0], applyForm.getElementsByClassName('minute')[0]]
        };
        ...
        if(err_flag){
            loading(true);
            errorHandling({message: `"${message}"를 입력하지 않았습니다. "${message}"를 입력해주세요.`, reload: false});
            return false;
	}
...
modal.querySelector("#symSubmit").addEventListener("click", (e) => {
    ...
    for(let box of checkboxes){
        if(box.checked){
            ...
            if(box.value == "그 외"){
                current.querySelector("#otherSymptomCover").setAttribute("hidden", true);
                other = false;
            }
        }
    }
...
about.addEventListener("click", (e) => {
    location.replace("https://docs.google.com/document/d/1degMd317t3KixcS9bujQ--YyTdfb7kVPpdHn9UZeeq8/edit?usp=sharing");
});
