...
const diags = "보건실휴식,타세놀,모드코,모드콜,모드코프,쎄로테정,지르텍,리놀,화이투벤큐노즈,화이투벤큐,부스코판플러스,부루펜,아나프록스정,탁센,그날엔,이지엔식스이브,이지엔식스,탁센레이디,정로환,베아제,훼스탈,마로이 신,나조린,아이투오,케프란,신폴에이,스타빅,알마겔,포타겔,coban,후시메드,밴드,에어파스,멸균안대,생리대,자가키트,거즈,탄력붕대,바세린,오라메디,페리덱스,알보칠,아프니벤큐,쎄레스톤지,제올라,버물리,아즈렌,하이맘번,드레싱밴드,생리식염수,애니클린,리도맥스,타세놀콜드,호올스,포도당캔디,애드빌,비코그린,베나치오,맥시롱,쌍화탕,콜대원,백초,써버쿨,타이레놀우먼,반창고,타벡스겔,아이스팩,핫팩,병원진료확인증,씨즈날 정,소독,파스,박트리안,생리식염수,케토핀프리,신폴에이,소렉신,생리조퇴".split(",");
/**
 * pursue
 * 0 = connect request
...
const makeTable = (data) => {
  ...
  delBtn.addEventListener("click", async (e) => {
    ...
    if (flag) {
      ...
      await webIO.send(
        form({ type: 9, stat: 1, header: "m", body: fetchInfo })
      );
    } else {
      return;
    }
...
const makeList = (data, num) => {
  ...
  btnX.addEventListener("click", async (e) => {
    ...
    if (flag) {
      await webIO.send(
        form({ type: 3, stat: 1, header: "m", body: {"uniq":data["uniq"], "time":data["time"]} })
      );
...
webIO.onopen = async () => {
  ...
  await webIO.send(form({ type: 0, header: "m" }));
...
webIO.onmessage = async (data) => {
  ...
  if (stat != 1) {
	...
	case "pong":
	  break;
        case 0:
          await webIO.send(form({ type: 1, header: "m" }));
          break;
        case 1:
          /*
                        waiterInfo의 구조
...
confirmModal
  ...
  .addEventListener("click", async (e) => {
    ...
    if (bedFlag) {
      ...
      bedNum--;
      await webIO.send(form({ type: 4, stat: 1, header: "m", body: -1 }));
      bedBtn.src = `./image.resize/bedRemain0${bedNum}.png`;
    }

    // content: id, number, name, sex, time, disease, treat object
    await webIO.send(form({ type: 7, stat: 1, header: "m", body: contents }));
...
diagBtn.addEventListener("click", async (e) => {
  ...
  diagBtn.src = diagPos
    ? "./image.resize/diagPos.png"
    : "./image.resize/diagImpos.png";
  await webIO.send(form({ type: 5, stat: 1, header: "m", body: diagPos }));
});
...
applyButton.addEventListener("click", async (e) => {
  ...
  if (flag) {
    await webIO.send(form({ type: 8, stat: 1, header: "m", body: ret }));
  } else {
    return;
  }
});
...
download.addEventListener("click", async (e) => {
  if (
    ...
    await webIO.send(form({ type: 10, stat: 1, header: "m" }));
  }
});
