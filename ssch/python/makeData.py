...
with mysql.cursor() as commander:
    ...
    for _ in range(90):
        ...
        sex = choice(["남", "여"])
        dise = choice(dis)
        time = f'2023.09.{str(randint(1, 4)).rjust(2, "0")} {str(randint(8, 17)).rjust(2, "0")}:{str(randint(0, 59)).rjust(2, "0")}'
        treat = "테스트"*randint(1, 5)
        temp = [number, name, sex, dise, time, treat]
        shuffle(temp)
        ha = hash160(''.join(temp))
    ...
    for _ in range(350):
        ...
        sex = choice(["남", "여"])
        dise = choice(dis)
        time = f'2023.08.{str(randint(1, 31)).rjust(2, "0")} {str(randint(8, 17)).rjust(2, "0")}:{str(randint(0, 59)).rjust(2, "0")}'
        treat = "테스트"*randint(1, 5)
        temp = [number, name, sex, dise, time, treat]
        shuffle(temp)
        ha = hash160(''.join(temp))
